...
_NOT_FOUND = "No class found."
_UNREACHABLE = "Couldn't reach CUNY Global Search. Try again in a moment."

# Hand-maintained. Bump these when the term you want pre-selected rolls over.
DEFAULT_TERM = "Fall Term"
DEFAULT_YEAR = 2026
...
async def index(request: Request):
    return templates.TemplateResponse(
        ...
        "index.html",
        {
            ...
            "year_options": [DEFAULT_YEAR, DEFAULT_YEAR + 1],
        },
    )
...
async def status(
    ...
    try:
        ...
        return {"ok": True, "found": False, "error": _NOT_FOUND}
    except ScrapeError as exc:
        log.warning("Status lookup scrape failed: %s", exc)
        return JSONResponse(
            {"ok": False, "found": False, "error": _UNREACHABLE}, status_code=502
        )
...
async def subscribe(request: Request, payload: SubscribeRequest):
    ...
    try:
        ...
        return JSONResponse({"ok": False, "error": _NOT_FOUND}, status_code=404)
    except ScrapeError:
        return JSONResponse({"ok": False, "error": _UNREACHABLE}, status_code=502)
    ...
    if not created:
        return {
            "ok": True,
            "already": True,
            "message": f"You're already tracking {full_name} (#{params.class_number}).\n"
